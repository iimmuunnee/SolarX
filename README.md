...
글로벌 배터리 3사(LG, Samsung, Tesla)의 스펙을 적용하여 **장기 시뮬레이션**을 수행한 결과입니다.
![Benchmark Graph](./images/benchmark_graph.png)
*(하단 그래프: 기준 모델(0) 대비 순수 추가 수익(Net Gain)을 시각화. 효율이 높은 삼성 SDI 모델이 시간이 지날수록 수익 격차를 벌리는 '복리 효과'를 확인)*

| Rank | 제조사 (Brand) | 누적 수익 (KRW) | 수익 개선율 | 분석 (Engineering Insight) |
| :--- | :--- | :--- | :--- | :--- |
| **🥇 1위** | **Samsung SDI** | **54,044,060원** | **+26.77%** | 충전 속도는 2위였으나, **업계 최고 효율(98.5%)** 덕분에 전력 손실이 가장 적어 최종 수익 1위 달성. |
| **🥈 2위** | **LG Energy Solution** | **53,847,424원** | **+26.31%** | 가장 빠른 충전 속도(2.0C)를 자랑했으나, 미세한 효율 차이로 인해 누적 수익에서 근소하게 2위 기록. |
| **🥉 3위** | **Tesla (4680)** | **53,457,184원** | **+25.39%** | 표준 성능으로 안정적인 수익을 냈으나, 프리미엄 모델(LG/Samsung) 대비 성능 열세 확인. |
| **-** | **기존 방식 (No ESS)** | 42,632,416원 | 0.00% | 비교 기준점 (Baseline) |

> **💡 결론:**
> 휴머노이드 로봇 충전 스테이션에 있어 '충전 속도(Speed)'도 중요하지만, 장기적인 운영 관점(OPEX)에서는 **'에너지 변환 효율(Efficiency)'이 수익성에 더 결정적인 영향**을 미친다는 사실을 입증했습니다. 
> 따라서 **Samsung SDI의 고효율 NCA 배터리**를 채택하는 것이 가장 경제적인 솔루션입니다.
...
* **v1.0: Basic Prediction Model**
    ...
    * 단순 알고리즘(싸게 사서 비싸게 팔기) 적용.
* **v2.0: Physics-Informed Model (Realism)**
    * 배터리의 물리적 제약(C-Rate, DoD, Efficiency)을 코드에 반영.
    * 현실 불가능한 충전 시나리오를 배제하여 신뢰도 확보.
* **v3.0: Pivot to Humanoid Station**
    * 전기차 캐즘 이후의 시장인 **'휴머노이드 로봇'** 으로 타겟 변경.
    * 로봇 구동에 필수적인 **'46파이 원통형 배터리(Tabless)'** 기술 도입.
    * 단일 모델을 넘어 **글로벌 3사(LG, Samsung, Tesla)** 의 기술력을 비교 분석.
