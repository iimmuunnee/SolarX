...
> **수익은 일사량에 정확히 선형 비례하지 않습니다.** 발전량이 많은 제주(고일사)는 선형 기대보다 0.84% 낮고, 적은 시애틀(저일사)은 2.43% 높습니다 — 발전량이 커질수록 고정 용량(2,280 kWh)·C-rate 한계로 잉여를 다 회수하지 못해 한계수익이 체감합니다. (발전량 계수를 조정한 가상 시나리오이며, 실제 지역 기후 데이터 기반 검증은 향후 과제입니다.)
