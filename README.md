...
2. **가상 가격으로 +33.29%였던 개선율이, KPX 실측 SMP를 넣자 +3.79%로 떨어졌다** — 낙관 편향을 실데이터로 정량 확인하고, 줄어든 수치를 그대로 보고했다(samsung 기준, `python scripts/bias_check.py`로 재현). → [핵심 결과](#2--핵심-결과-kpx-실측-데이터-기준)
...
|:---|:---|:---|:---|:---|
| 🥇 | **Samsung SDI** (NCA) | 약 5,904만 원 | **+3.79%** | 최고 효율(98.5%)로 왕복 손실 최소화 |
| 🥈 | LG Energy Solution (NCM) | 약 5,899만 원 | +3.70% | 고출력(2.0C)이나 효율 차이로 근소한 2위 |
| 🥉 | Tesla In-house (4680) | 약 5,867만 원 | +3.13% | 표준형 모델, 안정적이나 효율 열세 |
| — | Baseline (ESS 없음) | 약 5,688만 원 | 0.00% | 비교 기준점 (직판매만) |

> **측정 범위**: 평가 구간 2,592시간(108일) 테스트 분할 · 용량 2,280 kWh · `allow_grid_charge=True` · KPX 실측 SMP(2024~2025).
> **데이터 기간(SMP 2024~2025)과 평가 구간(108일 테스트 분할)은 다릅니다.** 수치는 이 가정에 종속적이므로 유효숫자를 줄여 표기했고, 원 단위 값과 개선율은 `python scripts/bias_check.py`로 그대로 재현됩니다.

**이 결과에서 배운 것 두 가지:**

- **가상 대비 실측의 간극**: 가상 고정 가격(낮 100원 / 그 외 200원)에서는 개선율이 +33.29%까지 나왔지만, 실제 SMP의 제한적인 가격 변동폭 안에서는 +3.79%로 줄었다(samsung 기준). 시뮬레이션의 낙관 편향은 실데이터를 넣기 전까지는 보이지 않는다. 이 쌍은 `python scripts/bias_check.py`로 재현된다.
...
> **위 그래프와 아래 표는 서로 다른 실험입니다 — 대비로 읽어야 합니다.**
> 위 그래프(`main.py` Part 2)는 **설비를 통째로 확장**한 경우입니다: 발전량과 함께 배터리 용량도 같은 계수로 키웁니다(`SamsungSDI(용량 × factor)`, `main.py:194`). 발전·저장이 함께 커지므로 결과가 **정확히 선형**입니다 — 그래프의 Jeju 약 7,675만 원 = Donghae 약 5,904만 원 × 1.3, Seattle 약 3,542만 원 = × 0.6 (원 단위로 일치).
> 아래 표는 반대로 **배터리를 2,280 kWh로 고정**하고 발전량만 키운 경우입니다(같은 설비를 일사량이 다른 부지에 놓았을 때). 그래서 결과가 선형에서 어긋나며, **그 어긋난 몫이 곧 배터리 용량이 병목이라는 증거**입니다: 발전량이 많은 제주는 고정 배터리가 잉여를 다 못 담아 선형보다 낮고(−0.84%), 발전량이 적은 시애틀은 배터리가 상대적으로 여유로워 높습니다(+2.43%).

| 시나리오 | 발전량 계수 | Samsung 누적 수익 | 선형 기대 대비 |
|:---|:---|:---|:---|
| 동해 (기준) | ×1.0 | 약 5,904만 원 | 기준 |
| 제주 (고일사) | ×1.3 | 약 7,610만 원 | **−0.84%** |
| 시애틀 (저일사) | ×0.6 | 약 3,629만 원 | **+2.43%** |

> **측정 조건**: 세 지역 모두 **동일 설비(용량 2,280 kWh)** 를 일사량이 다른 부지에 놓았다고 가정 · 실측 SMP · `region_factor`만 변경. 대표값은 수익 1위 Samsung SDI의 누적 수익. 재현: `python scripts/bias_check.py`.
> 표의 "선형 기대 대비"는 위 그래프(설비 전체 확장, 정확히 선형)를 기준으로 한 편차입니다. 이 표는 발전량 계수를 조정한 가상 시나리오이며, 실제 지역 기후 데이터 기반 검증은 향후 과제입니다.
